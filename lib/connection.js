...
module.exports = (function() {
  ...
  DbHelper.prototype.registerCollections = function() {
    ...
    async.auto({
        ...
        getEdgeCollections: ['ensureDB', function(next) {
          ...
          debug('getEdgeCollections::', edgeCollections)
        ...
        createMissingCollections: ['getCollections', function(next, results) {
          ...
          debug('currentCollections::', currentCollections)
          debug('missingCollections::', missingCollections)
  ...
  */
  DbHelper.prototype.join = function(collection, criteria, cb) {
    debug('join criteria:', criteria, 'this.graph.name:', this.graph.name);
    ...
    this.db.query(join_query, function(err, cursor) {
      ...
      cursor.map((v) => {
        ...
        if (criteria.select && criteria.select.length > 0) {
          ...
          debug('join() returning nv:', nv);
          return nv;
        }
        debug('join() returning v:', v);
        return v;
      })
      .then((vals) => {
        debug('join() vals:', vals);
