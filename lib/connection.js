...
module.exports = (function () {
  ...
  DbHelper.prototype.find = function (collection, options, cb) {
   var qb = optionsToQuery(collection, options), qbret = { _id: 'd._id', _key: 'd._key' };
    if (options.select) {
      _.each(options.select, function (ret) {
	qbret[ret] = 'd.' + ret;
      });
      qb = qb.return_(_.values(qbret));
    } else {
      qb = qb.return_('d');
    }
    ...
    this.db.query(qb.toAQL(), function (err, cursor) {
      ...
      cursor.all(function (err, vals) {
	if (options.select) {
	  var rvals = [], rkeys = _.keys(qbret), rval;
	  _.each(vals, function (val) {
	    rval = {};
	    _.each(val, function (field, index) {
	      rval[rkeys[index]] = field;
	    });
	    rvals.push(rval);
	  });
	  return cb(err, fixIds(rvals));
	}
	return cb(err, fixIds(vals));
      });
    });
      ...
      coll.save(options, function (err, res)
		{
		  ...
		  var qb = optionsToQuery(collection, { 'where': { '_key' : res._key }, 'limit': 1 });
		  qb = qb.return_('d');
