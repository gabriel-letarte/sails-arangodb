...
//        return cb(err, fixIds(vals));
      ...
      _.each(options.where, function(v, k) {
        ...
        if (v && typeof v === 'object') {
          ...
          if (_.isArray(v)) {
            ...
            _.each(v, (vv, kk) => {
              debug('optionsToQuery vv:', typeof vv, vv);
              v = vv;
              switch(kk) {
                case 'contains':
                  operator = 'LIKE';
                ...
                default:
                  throw new Error('Unsupported operator: ' + kk);
              }
            });
          }
        }

        debug(`options.where: '${v}' ${operator} ${k}`);
  ...
  // Deprecated in favour of processor cast()
  var fixIds = function(vals) {
    debug('Call to deprecated fixIds() - see processor cast() instead: stack:', (new Error('stack')).stack);
    _.each(vals, function(v, k) {
      vals[k]['id'] = vals[k]['_key'];

      // Fixed in processor cast()
//      if (vals[k]['createdAt']) {
...
//      }
  ...
  DbHelper.prototype.applyFunctions = function (options, qb) {
    ...
    _.each(options, function (v, k) {
      if (_.includes(['where', 'sort', 'limit', 'select'], k)) {
        return;
      }
      funcs[k] = v;
    });
...
//        })));
  ...
  DbHelper.prototype.drop = function(collection, relations, cb) {
    debug(`drop() collection: ${collection}, relations: ${relations}, cb: ${cb}`);
  ...
  DbHelper.prototype.join = function(collection, criteria, cb) {
    ...
    _.each(criteria.joins, function(join) {
      ...
      if (me.graph.name) {
        ...
        // Fallback to relational join
        ret += `, "${join.alias}": (FOR ${join.alias} IN ${join.child} FILTER (${join.alias}.${join.childKey} == d.${join.alias}) RETURN ${join.alias})`;
      }
    });
    ret += ' }';
    var join_query = qb + ret;
    ...
    this.db.query(join_query, function(err, cursor) {
      ...
      .then((vals) => {
        debug('query join response:', vals.length, 'documents returned for query:', join_query);
        return cb(null, fixIds(_.map(vals, function(item) {
          var bo = item.d;
//          debug('bo b4 select:', bo);
          _.each(criteria.joins, function(join) {
...
//          debug('join returning bo:', bo);
  ...
  var connect = function(connection, collections) {
    ...
    debug('connect() collections:', collections);
