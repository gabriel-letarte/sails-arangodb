...
module.exports = (function() {
  ...
  var adapter = {
    ...
    find: function(connectionName, collectionName, searchCriteria, cb) {
      ...
      var qsort = '';
      if (searchCriteria.sort && !_.isEmpty(searchCriteria.sort)) {
        qsort = 'SORT ' + _.map(searchCriteria.sort, (v, k) => {
          return `x.${k} ${v < 0 ? 'DESC' : 'ASC'}`;
        }).join(', ');
      }
      ...
      var query = [qfor, qsort, qfilter, qlimit, qreturn].join(' ');
    ...
    */
    ...
    create: function(connectionName, collectionName, data, cb) {
      ...
      return col.save(data, true, function (err, doc) {
        ...
        // fix _id -> id - now in processor cast()
//        doc.new.id = doc.new._key;
    ...
    update: function(connectionName, collectionName, searchCriteria , values , cb) {
      debug('update() connectionName:', connectionName, 'collectionName:', collectionName, 'searchCriteria:', searchCriteria);
...
//      debug('Still WIP');
//      debug(searchCriteria);
//      debug(searchCriteria.where);
//      debug(values);
//      debug([qfor, qfilter, qupdate, qreturn].join(' '));
//      debug('-------------------');

      return db.query([qfor, qfilter, qupdate, qreturn].join(' '), function(err, r) {
        if (err) {
...
//        return cb(null, r._result)
        var processor = new Processor(connections[connectionName].collections);
        debug('update err:', err, 'returning r._result:', r._result);
        cb(null, processor.cast(collectionName, {_result: r._result}));
      });
    },
    ...
    */
    _queryFormatter: function(searchCriteria, separator, begin) {
      ...
      for (var k in searchCriteria) {
        ...
        if (searchCriteria.hasOwnProperty(k)) {
          ...
          // ArangoDB has '_key' instead of 'id'
          if (k === 'id'){
            k_override = '_key';
          }
      ...
      debug('_queryFormatter() returning r:', r);
      return r;
    },
    ...
    join: function(connection, collection, criteria, cb) {
      ...
      connections[connection].join(collection, criteria, function (err, r) {
        ...
        //debug('join before cast results:', r);
        var cast_r = processor.cast(collection, {_result: r});
        //debug('join after cast results:', cast_r);
        return cb(null, cast_r);
      });
    }
